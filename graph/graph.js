...
*/
// { useState, useEffect, useRef} from 'http://unpkg.com/preact/hooks/dist/hooks.module.js?module';
...
fetch('./output/graph.json')
  ...
  /*
        ...
        nodes.forEach((node) => {
            ...
            testTable[node.label] = 0;
            switch (node.label) {
                case 'Domain': {
                    node.type = 'rect';
                    node.size = [25, 15];
                    node.style.fill = '#BA55D3';
                ...
                case 'ASN': {
                    node.type = 'triangle';
                    node.size = 15;
                    node.style.fill = '#FF69B4';
                    node.style.opacity = 0.6;
                    break;
                }
                default: {
                    if (node.label.length > 6 && node.label.substr(0, 6) == 'Domain') {
                        node.type = 'rect';
                        node.size = [25, 15];
                        node.style.fill = '#BA55D3';
                        node.style.opacity = 0.6;
                    }
                    else if (node.label.substr(0, 4) == 'Cert') {
                        node.type = 'diamond';
                        node.size = 15;
                        node.style.fill = '#FFA500';
                        node.style.opacity = 0.6;
        ...
        //
        edges.forEach((edge) => {
            if (edge.sytle === undefined) edge.style = {};
            edge.style.lineWidth = 0.7;
            edge.style.strokeOpacity = 0.1;
            edge.style.stroke = `l(0) 0:${llightOrange16} 1:${llightBlue16}`;
            //
            if (!edge.style) {
                edge.style = {};
            }
        ...
        modes: {
          ...
          'zoom-canvas',
          {
            ...
            shouldUpdate: (edge) => true,
          },
        ],
      },
        layout: {
            type: 'force', // 设置布局算法为 force
            linkDistance: 110, // 设置边长为 100
            preventOverlap: true, // 设置防止重叠
        },
    });
        edgeBundling.bundling(remoteData);
